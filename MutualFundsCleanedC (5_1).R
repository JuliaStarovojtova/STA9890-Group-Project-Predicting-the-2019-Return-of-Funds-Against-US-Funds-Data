...
##Betas with names
#Coefficients Ridge
beta.coef_rid=data.frame(names(x[1,]),as.vector(model_ridge_full$beta))
colnames(beta.coef_rid)=c("feature","value")

#Coefficients Lasso
beta.coef_las=data.frame(names(x[1,]),as.vector(model_lasso_full$beta))
colnames(beta.coef_las)=c("feature","value")

#Coefficients El-net
beta.coef_elnet=data.frame(names(x[1,]),as.vector(model_elnet_full$beta))
colnames(beta.coef_elnet)=c("feature","value")
beta.coef_elnet%>%arrange(beta.coef_elnet)
